...
export default function NormsPage() {
  const { data: norms, mutate } = useSWR<Norms>("/api/norms", fetcher, {
  ...
  const html = norms?.content ? marked.parse(norms.content) : "";
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex items-start justify-between mb-8 gap-4 flex-wrap">
        <div>
          <h1 className="text-3xl font-bold text-navy-900 tracking-tight">Norms</h1>
          <p className="text-slate-500 mt-1">How we work together</p>
        </div>
        {!editing && (
      ...
      {!editing ? (
        norms?.content ? (
          ...
          <p className="text-slate-400 italic">No norms yet.</p>
        )
      ) : (
        <div>
          <textarea
      ...
      {showPasswordPrompt && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-navy-950/60 backdrop-blur-sm">
          <div className="card p-8 max-w-md w-full mx-4">
            ...
            <p className="text-slate-500 text-sm mb-4">Only the admin can edit norms.</p>
