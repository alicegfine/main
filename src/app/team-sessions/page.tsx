...
function SessionCard({ session, allSessions, userName }: { session: Session; allSessions: Session[]; userName?: string }) {
  ...
  return (
    <Link href={`/session/${session.id}`}>
      <div className={`card p-4 hover:shadow-md transition-shadow cursor-pointer group ${conflict ? "border-amber-400 bg-amber-50/50" : isAttending ? "border-2 border-navy-500 bg-navy-50/40" : ""}`}>
        {conflict && (
